...
for (iFile=0; iFile < files.length; iFile++) {
	...
	selectWindow(files[iFile]);
	close();
	selectWindow("MAX_"+files[iFile]);
	run("Duplicate...", "duplicate");
	openWindows = getAllOpenWindows("names");
	run("Concatenate...", "  title=concat open image1="+openWindows[0]+" image2="+openWindows[1]+" image3=[-- None --]");
