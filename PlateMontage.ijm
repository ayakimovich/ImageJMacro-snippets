...
//change
print ("start");
ReadPath = "H:\\101125-AB-blocking_Plate_18";
SavePath = "H:\\101125-AB-blocking_Plate_18\\Montage";
...
//print (RowLetterArray.length);
	for(i=0; i<=RowLetterArray.length-1; i++){
		for(j=0; j<=StitchX_plate-1; j++){
			...
			run("Image Sequence...", "open="+ReadPath+File.separator+"TimePoint_"+k+1+File.separator+" number=6913 starting=1 increment=1 scale=100 file=[] or=.*"+RowLetterArray[i]+jNumber+"_s*.._w2_Thumb.TIF sort");	
